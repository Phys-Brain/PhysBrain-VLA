...
class PhysBrainVLA(baseframework):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass with PhysBrainVLA + LangForce support.
        
        If use_langforce=True:
                    - Prior Forward: left brain (V+L) + right brain (V+A+L)
                    - Posterior Forward: left brain (V+L) + right brain (V+L+A)
          - Compute LLR loss and combined action loss
        Else:
                    - Single forward (original PhysBrainVLA behavior)
        """
        ...
        # ===== LangForce Mode: Dual Forward =====
        if self.use_langforce and self.training:
            return self._forward_langforce(
                batch_images, instructions, actions, state, embodiment_id
            )
        
        # ===== Original PhysBrainVLA Mode: Single Forward =====
        # Step 1: Build separate inputs for frozen and trainable VLMs
        frozen_inputs, trainable_inputs, state_tensor, embodiment_id_tensor = self.qwen_vl_interface.build_qwenvl_inputs(
            images=batch_images, 
            instructions=instructions,
            state=state,
            embodiment_id=embodiment_id,
            mode='posterior',  # Default to posterior mode
        )
        
        # Return only hidden states from the selected layer to save memory
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # selected_hidden_state: [B, seq_len, H]
            selected_hidden_state = qwenvl_outputs.hidden_states[0]   # Only one element: hidden state from selected layer

        # Step 4: Action Expert Forward and Loss
        with torch.autocast("cuda", dtype=torch.float32):
            actions = torch.tensor(
                np.array(actions), device=selected_hidden_state.device, dtype=selected_hidden_state.dtype
            )  # [B, T_full, action_dim]
            
            # Align Action Dimension
            model_action_dim = self.config.framework.action_model.action_dim
            if actions.shape[-1] != model_action_dim:
                # logger.warning(f"Action dimension mismatch: Data has {actions.shape[-1]}, Model expects {model_action_dim}. Truncating/Adjusting...")
                if actions.shape[-1] > model_action_dim:
                    actions = actions[..., :model_action_dim]
                else:
                    raise ValueError(f"Action dimension mismatch: Data has {actions.shape[-1]}, Model expects {model_action_dim}. Cannot automatically adapt.")
            ...
            selected_hidden_state_repeated = selected_hidden_state.repeat(repeated_diffusion_steps, 1, 1)
            
            state_repeated = None
            if state is not None:
                state = torch.tensor(
                    np.array(state), device=selected_hidden_state.device, dtype=selected_hidden_state.dtype
                )
                state_repeated = state.repeat(repeated_diffusion_steps, 1, 1)

            # (B, chunk_len, action_dim)
            action_loss = self.action_model(
                selected_hidden_state_repeated, 
    ...
    def _forward_langforce(
        ...
        """
        LangForce dual-forward implementation.
        
        Prior Forward: left brain (V+L) + right brain (V+A+L) -> p(a|v), log p(L|V,A)
        Posterior Forward: left brain (V+L) + right brain (V+L+A) -> π(a|v,l), log p(L|V)
        """
        # ===== Step 1: Prior Forward (V + A + L) =====
        # Left brain: V + L (without action token)
        # Right brain: V + A + L (action token before language)
        prior_frozen_inputs, prior_trainable_inputs, _, _ = self.qwen_vl_interface.build_qwenvl_inputs(
            images=batch_images,
            instructions=instructions,
            state=state,
            embodiment_id=embodiment_id,
            mode='prior',  # Action token BEFORE language
        )
        
        with torch.autocast("cuda", dtype=torch.bfloat16):
            prior_outputs, prior_logits = self.qwen_vl_interface(
                is_vla=True,
                frozen_inputs=prior_frozen_inputs,
                trainable_inputs=prior_trainable_inputs,
                state_tensor=None,
                embodiment_id_tensor=None,
                selected_layer=self.vla_config.hidden_states_selected_layer,
                output_attentions=False,
                output_hidden_states=True,
                return_dict=True,
            )
            # prior_action_hidden: [B, N, H]
            prior_action_hidden = prior_outputs.hidden_states[0]
            # prior_logits: [B, S, V]
        
        # Get action start positions for prior
        prior_action_starts = []
        for b in range(prior_trainable_inputs['input_ids'].shape[0]):
            start = self.qwen_vl_interface._get_action_block_start(prior_trainable_inputs['input_ids'][b])
            prior_action_starts.append(start)
        prior_action_starts = torch.tensor(prior_action_starts, device=prior_trainable_inputs['input_ids'].device, dtype=torch.long)
        
        # ===== Step 2: Posterior Forward (V + L + A) =====
        # Left brain: V + L (same as above)
        # Right brain: V + L + A (action token after language)
        post_frozen_inputs, post_trainable_inputs, _, _ = self.qwen_vl_interface.build_qwenvl_inputs(
            images=batch_images,
            instructions=instructions,
            state=state,
            embodiment_id=embodiment_id,
            mode='posterior',  # Action token AFTER language
        )
        
        with torch.autocast("cuda", dtype=torch.bfloat16):
            post_outputs, post_logits = self.qwen_vl_interface(
                is_vla=True,
                frozen_inputs=post_frozen_inputs,
                trainable_inputs=post_trainable_inputs,
                state_tensor=None,
                embodiment_id_tensor=None,
                selected_layer=self.vla_config.hidden_states_selected_layer,
                output_attentions=False,
            ...
            # post_action_hidden: [B, N, H]
            post_action_hidden = post_outputs.hidden_states[0]
            # post_logits: [B, S, V] - detach to prevent gradient flow
            post_logits = post_logits.detach()
        
        # Get action start positions for posterior
        post_action_starts = []
        for b in range(post_trainable_inputs['input_ids'].shape[0]):
            start = self.qwen_vl_interface._get_action_block_start(post_trainable_inputs['input_ids'][b])
            post_action_starts.append(start)
        post_action_starts = torch.tensor(post_action_starts, device=post_trainable_inputs['input_ids'].device, dtype=torch.long)
        
        # ===== Step 3: Compute LLR Loss =====
        llr_loss = self.qwen_vl_interface._compute_language_llr_from_boundaries(
            prior_logits=prior_logits,
            posterior_logits=post_logits,
            prior_input_ids=prior_trainable_inputs['input_ids'],
            posterior_input_ids=post_trainable_inputs['input_ids'],
            prior_action_starts=prior_action_starts,
            posterior_action_starts=post_action_starts,
            use_hard_token_llr=self.use_hard_token_llr,
            hard_token_k=self.hard_token_k,
            assert_lang_span_match=self.assert_lang_span_match,
        )
        
        # ===== Step 4: Action Head Losses =====
        with torch.autocast("cuda", dtype=torch.float32):
            actions_t = torch.tensor(
                np.array(actions), device=prior_action_hidden.device, dtype=prior_action_hidden.dtype
            )
            
            # Align Action Dimension
            model_action_dim = self.config.framework.action_model.action_dim
            if actions_t.shape[-1] != model_action_dim:
                if actions_t.shape[-1] > model_action_dim:
                    actions_t = actions_t[..., :model_action_dim]
                else:
                    raise ValueError(f"Action dimension mismatch: Data has {actions_t.shape[-1]}, Model expects {model_action_dim}.")
            ...
            if state is not None:
                state_tensor = torch.tensor(
                    np.array(state), device=prior_action_hidden.device, dtype=prior_action_hidden.dtype
                )
            
            actions_target_repeated = actions_target.repeat(repeated_diffusion_steps, 1, 1)
            
            # Detach prior condition (optional)
            if self.detach_prior_cond:
                prior_cond_base = prior_action_hidden.detach()
            else:
                prior_cond_base = prior_action_hidden
            
            prior_cond = prior_cond_base.repeat(repeated_diffusion_steps, 1, 1).float()
            post_cond = post_action_hidden.repeat(repeated_diffusion_steps, 1, 1).float()
            state_repeated = state_tensor.repeat(repeated_diffusion_steps, 1, 1) if state_tensor is not None else None
            
            prior_action_loss = self.action_model(prior_cond, actions_target_repeated, state_repeated)
            post_action_loss = self.action_model(post_cond, actions_target_repeated, state_repeated)
        
        # ===== Step 5: Total Loss =====
        # Total = (1 - prior_weight) * post_loss + prior_weight * prior_loss - kl_weight * llr
        total_loss = (
            (1.0 - self.prior_loss_weight) * post_action_loss
            + self.prior_loss_weight * prior_action_loss
            - self.kl_weight * llr_loss
        )
        
        return {
            "action_loss": total_loss,
            "post_action_loss": post_action_loss.detach(),
            "prior_action_loss": prior_action_loss.detach(),
